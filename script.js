    ...
    if (notationType === 'ratio') {
        ...
        if (ratioParts.length !== 4 || ratioParts.some(isNaN)) {
            ...
            for (let i = 0; i < ratioParts.length; i++) {
                ...
                const currentNumerator = ratioParts[i];
                const currentDenominator = fundamentalOfChord;
                
                // Multiply current ratio by base ratio
                const effectiveNumerator = currentNumerator * baseNum;
                const effectiveDenominator = currentDenominator * baseDen;
